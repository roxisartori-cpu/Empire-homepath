...
app.get('/api/admin/stats', requireAdminKey, async (req, res) => {
  try {
    if (!process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SECRET_KEY.includes('placeholder') || process.env.STRIPE_SECRET_KEY.includes('your_stripe')) {
      return res.status(503).json({ error: 'Stripe is not configured. Set STRIPE_SECRET_KEY on the backend.' });
    }
    ...
    for (const sub of activeSubs) {
      ...
      if (name.toLowerCase().includes('individual') || name === 'Individual') planCounts.Individual += 1;
      else if (name.toLowerCase().includes('professional') || name === 'Professional') planCounts.Professional += 1;
      else if (name.toLowerCase().includes('enterprise') || name === 'Enterprise') planCounts.Enterprise += 1;
    ...
    const subscribers = subscriptions
      ...
      .map((sub) => {
        ...
        else if (sub.status === 'trialing') statusLabel = 'Active';
    ...
    const recentPayments = succeededCharges.slice(0, 10).map((charge) => {
      const email =
