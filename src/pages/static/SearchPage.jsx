...
@media(max-width:600px){
  ...
  .toggle-btn{padding:7px 12px;font-size:11px;}
      ...
      <nav>
        ...
        <ul className="nav-links"><li><Link to="/#why">Why It Works</Link></li><li><Link to="/#features">Features</Link></li><li><Link to="/#who">Who It's For</Link></li><li><Link to="/#pricing">Pricing</Link></li></ul>
      ...
      <div className="page-wrap">
        ...
        <div className="page-header">
          <div className="header-eyebrow">Program Search</div>
          <h1 className="page-title">Find Programs for <span>Your Buyer</span></h1>
          <p className="page-subtitle">Answer a few quick questions to match your client to every verified homebuyer assistance program available in their county. No jargon, no fine print.</p>
          <div className="header-note">
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><circle cx="6" cy="6" r="5" stroke="#C9A84C" stroke-width="1.2"></circle><path d="M6 5v4M6 3.5v.5" stroke="#C9A84C" stroke-width="1.2" stroke-linecap="round"></path></svg>
        ...
        <div className="form-panel">
          <div className="form-section-title">Tell Us About Your Buyer</div>
      
          {/* County */}
          <div className="field-group">
            <div className="field-label">County They're Buying In</div>
            <div className="select-wrap">
              <select className="field-select" id="f-county">
          ...
          {/* City / Town */}
          <div className="field-group">
            <div className="field-label">City or Town (Optional)</div>
            <input type="text" className="field-input" id="f-city" placeholder="e.g. Newburgh, White Plains…"/>
            <div className="field-hint">Helps match city-specific programs</div>
          </div>
          ...
          <div className="field-group">
            ...
            <div className="select-wrap">
              <select className="field-select" id="f-income">
          ...
          <div className="field-group">
            ...
            <div className="select-wrap">
              <select className="field-select" id="f-price">
          ...
          {/* Buyer Type */}
          <div className="field-group">
            <div className="field-label">Buyer Status</div>
            <div className="field-hint" style={{marginBottom: '8px'}}>Select all that apply</div>
            <div className="toggle-group" id="buyerTypeGroup">
              <button className="toggle-btn" data-val="firsttime" onClick={() => { try { (0, eval)("toggleBtn(this,'buyerTypeGroup')") } catch (e) { /* noop */ } }}>First-Time Buyer</button>
              <button className="toggle-btn" data-val="veteran" onClick={() => { try { (0, eval)("toggleBtn(this,'buyerTypeGroup')") } catch (e) { /* noop */ } }}>Veteran</button>
              <button className="toggle-btn" data-val="teacher" onClick={() => { try { (0, eval)("toggleBtn(this,'buyerTypeGroup')") } catch (e) { /* noop */ } }}>Educator</button>
              <button className="toggle-btn" data-val="lowmod" onClick={() => { try { (0, eval)("toggleBtn(this,'buyerTypeGroup')") } catch (e) { /* noop */ } }}>Low-Mod Income</button>
              <button className="toggle-btn" data-val="disability" onClick={() => { try { (0, eval)("toggleBtn(this,'buyerTypeGroup')") } catch (e) { /* noop */ } }}>Disability</button>
            </div>
          </div>
          ...
          <div className="field-group">
            ...
            <div className="select-wrap">
              <select className="field-select" id="f-hhsize">
          ...
          <div className="field-group">
            <div className="field-label">Program Types Interested In</div>
            <div className="field-hint" style={{marginBottom: '8px'}}>Leave blank to show all</div>
            <div className="toggle-group" id="progTypeGroup">
              <button className="toggle-btn" data-val="grant" onClick={() => { try { (0, eval)("toggleBtn(this,'progTypeGroup')") } catch (e) { /* noop */ } }}>Grants</button>
              <button className="toggle-btn" data-val="dpa" onClick={() => { try { (0, eval)("toggleBtn(this,'progTypeGroup')") } catch (e) { /* noop */ } }}>Down Payment</button>
              <button className="toggle-btn" data-val="forgivable" onClick={() => { try { (0, eval)("toggleBtn(this,'progTypeGroup')") } catch (e) { /* noop */ } }}>Forgivable</button>
              <button className="toggle-btn" data-val="rate" onClick={() => { try { (0, eval)("toggleBtn(this,'progTypeGroup')") } catch (e) { /* noop */ } }}>Rate Buydown</button>
            </div>
          </div>
      
          <div className="form-divider"></div>
      
          <button className="btn-search" onClick={() => { try { (0, eval)("runSearch()") } catch (e) { /* noop */ } }}>
          ...
          <button className="btn-reset" onClick={() => { try { (0, eval)("resetSearch()") } catch (e) { /* noop */ } }}>Clear All Filters</button>
        </div>
        ...
        <div className="results-panel" id="resultsPanel">
          ...
          {/* Empty state */}
          <div className="results-empty" id="stateEmpty">
            <div className="empty-icon">🔑</div>
            <div className="empty-title">Ready to search</div>
            <div className="empty-sub">Select a county and household income to find every matching homebuyer assistance program in seconds.</div>
          </div>
      
          {/* Loading state */}
          <div className="results-loading" id="stateLoading">
            <div className="loading-spinner"></div>
            <div className="loading-text">Searching programs…</div>
          </div>
      
          {/* Results header */}
          <div className="results-header" id="resultsHeader">
            <div className="results-summary">
              <div className="results-count-big" id="rCountBig">0</div>
              <div className="results-count-label" id="rCountLabel">Programs found</div>
            </div>
            <div className="results-county-tag" id="rCountyTag">
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M6 1a3.5 3.5 0 0 1 3.5 3.5C9.5 7.5 6 11 6 11S2.5 7.5 2.5 4.5A3.5 3.5 0 0 1 6 1Z" stroke="#C9A84C" stroke-width="1.2"></path><circle cx="6" cy="4.5" r="1.2" stroke="#C9A84C" stroke-width="1.2"></circle></svg>
              <span id="rCountyName">—</span>
            </div>
          </div>
      
          {/* Filter pills */}
          <div className="results-filters" id="resultsFilters">
            <span className="results-filter-label">Filter:</span>
            <div className="filter-pill active" onClick={() => { try { (0, eval)("filterResults('all',this)") } catch (e) { /* noop */ } }}>All</div>
            <div className="filter-pill" onClick={() => { try { (0, eval)("filterResults('grant',this)") } catch (e) { /* noop */ } }}>Grants</div>
            <div className="filter-pill" onClick={() => { try { (0, eval)("filterResults('dpa',this)") } catch (e) { /* noop */ } }}>Down Payment</div>
            <div className="filter-pill" onClick={() => { try { (0, eval)("filterResults('forgivable',this)") } catch (e) { /* noop */ } }}>Forgivable</div>
            <div className="filter-pill" onClick={() => { try { (0, eval)("filterResults('rate',this)") } catch (e) { /* noop */ } }}>Rate Programs</div>
          </div>
      
          {/* HUD note */}
          <div className="hud-note" id="hudNote">
            <div className="hud-note-label">Important — HUD First-Time Buyer Definition</div>
            <div className="hud-note-text">Your buyer may qualify as a <strong>first-time homebuyer</strong> even if they've owned before — the HUD definition includes anyone who has <strong>not owned a primary residence in the past 3 years.</strong> This opens eligibility to many more programs.</div>
          </div>
          ...
          <div className="pro-gate" id="proGate">
            <div className="pro-gate-title">🔒 Unlock All <span id="gateCount">0</span> Programs</div>
            <div className="pro-gate-sub">You're seeing a preview. Subscribe to Empire HomePath to access every verified program with full eligibility details, contact information, and PDF export.</div>
            <button className="btn-subscribe" onClick={() => { try { (0, eval)("window.__spaNavigate('/#pricing')") } catch (e) { /* noop */ } }}>View Subscription Plans</button>
          </div>
